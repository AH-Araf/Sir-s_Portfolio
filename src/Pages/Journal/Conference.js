...
const Conference = () => {
    ...
    useEffect(() => {
        fetch('https://saha-reno-portfolio-backend.onrender.com/article')
            ...
            .then(data => {
                console.log(data);
                const filteredConferencePaper = data.filter(article => article.types === 'Conference Paper');
                setConferencePaper(filteredConferencePaper);
    ...
    return (
        <div>
            <section className='paper-body'>
                ...
                {!loading && conferencePaper.map(article => (
